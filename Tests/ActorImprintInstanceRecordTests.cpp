...
namespace
{
	...
	bool RejectRecord(const json& source, ActorImprintInstanceRecordErrorCode expected)
	{
		...
		return !ActorImprintInstanceRecordReader::Read(source, sentinel, &error) &&
			sentinel.assetGuid == before && error.code == expected && !error.message.empty();
	}
	...
	void TestRecordCodec()
	{
		...
		ActorImprintInstanceRecordError error;
		Check(ActorImprintInstanceRecordReader::Read(source, record, &error),
			"Instance record reader accepts the complete schema");
		Check(record.actorGuids.size() == 2 && record.actorGuids[0].localObjectId == 10 &&
			record.propertyOverrides.size() == 2 && record.propertyOverrides[0].targetLocalObjectId == 10,
		...
		Check(ActorImprintInstanceRecordWriter::Write(record, encoded, &error),
			"Validated Instance record writes successfully");
		...
		Check(ActorImprintInstanceRecordReader::Read(encoded, repeated) &&
			ActorImprintInstanceRecordWriter::Write(repeated, repeatedJson) && repeatedJson.dump() == encoded.dump(),
			"Instance record read-write-read-write is byte-stable");
		Check(encoded["propertyOverrides"][0]["properties"]["/target"].is_null(),
			"Explicit null remains an Override value");

		json noOverrides = source;
		noOverrides.erase("propertyOverrides");
		ActorImprintSerializedInstanceRecord empty;
		json normalized;
		Check(ActorImprintInstanceRecordReader::Read(noOverrides, empty) && empty.propertyOverrides.empty() &&
			ActorImprintInstanceRecordWriter::Write(empty, normalized) && !normalized.contains("propertyOverrides"),
			...
			"externalParentActorGuid", "actorGuids" })
		{
			json bad = source;
			bad.erase(field);
			Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidSchema),
				"Required Instance field is rejected: " + field);
		}
		json bad = source;
		bad["unknown"] = true;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidSchema), "Unknown Instance field is rejected");
		bad = source; bad["assetGuid"] = nullptr;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidAssetGuid), "Null Asset GUID is rejected");
		bad = source; bad["assetGuid"] = source["assetGuid"].get<std::string>() + std::string("\0suffix", 7);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidAssetGuid), "Asset GUID with embedded NUL is rejected");
		bad = source; bad["sourceDefinitionRevision"] = "bad";
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidDefinitionRevision), "Malformed revision is rejected");
		bad = source; bad["rootActorGuid"] = source["rootActorGuid"].get<std::string>() + std::string("\0suffix", 7);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidRootActorGuid), "Root GUID with embedded NUL is rejected");
		bad = source; bad["externalParentActorGuid"] = 10;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidExternalParentActorGuid), "External parent accepts only null or GUID");
		bad = source; bad["externalParentActorGuid"] = source["rootActorGuid"].get<std::string>() + std::string("\0suffix", 7);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidExternalParentActorGuid), "External parent GUID with embedded NUL is rejected");
		bad = source; bad["actorGuids"] = json::object();
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Actor mapping must be an array");
		bad = source; bad["actorGuids"][0]["localObjectId"] = 0;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Zero Actor LocalObjectID is rejected");
		bad = source; bad["actorGuids"][0]["localObjectId"] = -1;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Negative Actor LocalObjectID is rejected");
		bad = source; bad["actorGuids"][0]["localObjectId"] = 10;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Duplicate Actor LocalObjectID is rejected");
		bad = source; bad["actorGuids"][0]["actorGuid"] = bad["actorGuids"][1]["actorGuid"];
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Duplicate Actor GUID is rejected");
		bad = source; bad["actorGuids"][0]["actorGuid"] =
			source["actorGuids"][0]["actorGuid"].get<std::string>() + std::string("\0suffix", 7);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidActorGuidMapping), "Actor GUID with embedded NUL is rejected");
		bad = source; bad["rootActorGuid"] = GuidGenerator::Generate().ToString();
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidRootActorGuid), "Root GUID must occur in the mapping");

		bad = source; bad["propertyOverrides"] = json::object();
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride), "Property Overrides must be an array");
		bad = source; bad["propertyOverrides"][0]["targetLocalObjectId"] = 0;
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride), "Zero Override target is rejected");
		bad = source; bad["propertyOverrides"].push_back(bad["propertyOverrides"][0]);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride), "Duplicate Override target is rejected");
		bad = source; bad["propertyOverrides"][0]["properties"] = json::array();
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride), "Override properties must be an object");
		for (const std::string invalid : { "", "name", "/~2", "/a//b" })
		{
			bad = source;
			bad["propertyOverrides"][0]["properties"] = json::object({ { invalid, true } });
			Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride),
				"Malformed JSON Pointer is rejected: " + invalid);
		}
		bad = source;
		bad["propertyOverrides"][0]["properties"] = { { "/nested", json::object() }, { "/nested/label", "x" } };
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride),
			"Ancestor and descendant paths are rejected independently of order");
		bad = source;
		bad["propertyOverrides"][0]["properties"]["/weight"] = (std::numeric_limits<double>::quiet_NaN)();
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride),
			"Non-finite Override values are rejected at the persistence boundary");
		bad = source;
		bad["propertyOverrides"][0]["properties"]["/payload"] = json::binary({ 1, 2, 3 });
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride),
			"Binary Override values are rejected at the JSON persistence boundary");
		bad = source;
		bad["propertyOverrides"][0]["properties"]["/label"] = std::string("\xff", 1);
		Check(RejectRecord(bad, ActorImprintInstanceRecordErrorCode::InvalidPropertyOverride),
			"Invalid UTF-8 Override strings are rejected at the JSON persistence boundary");
		...
		Check(!ActorImprintInstanceRecordWriter::Write(invalidWriter, writerSentinel, &error) && writerSentinel == beforeWriter,
	...
	void TestDiff()
	{
		...
		ActorImprintPropertyOverrideTarget target;
		ActorImprintPropertyOverrideError error;
		Check(ActorImprintPropertyOverrides::Diff(ProbeMetadata(), 10, defaults, defaults, target, &error) &&
			target.targetLocalObjectId == 10 && target.properties.empty(),
		...
		Check(ActorImprintPropertyOverrides::Diff(ProbeMetadata(), 10, defaults, current, target, &error) &&
			target.properties.size() == 3 && target.properties[0].path.ToString() == "/a~1b~0c" &&
			target.properties[1].path.ToString() == "/position" && target.properties[1].value.is_array() &&
		...
		Check(!ActorImprintPropertyOverrides::Diff(ProbeMetadata(), 10, defaults, current, sentinel, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::CurrentSchemaMismatch &&
			sentinel.targetLocalObjectId == 99 && sentinel.properties.size() == 1,
		...
		Check(!ActorImprintPropertyOverrides::Diff(ProbeMetadata(), 10, invalidDefault, defaults, sentinel, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::InvalidDefaultSchema,
			"Invalid default schema is distinguished from current mismatch");
	}

	bool FullyDeserialize(const json& properties, ReflectionError* outError = nullptr)
	{
		OverrideProbe probe;
		...
		return ReflectionDeserializer::Deserialize(ProbeMetadata(), typeid(OverrideProbe), properties, &probe, context, outError);
	}
	...
	void TestMergeAndFullValidation()
	{
		...
		Check(!ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, missing,
			ActorImprintOverrideRevisionRelation::Same, completed, &applied, &migration, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::MissingProperty && completed == beforeCompleted &&
			applied.size() == 1 && migration.applied == 7,
		...
		Check(ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, migrated,
			ActorImprintOverrideRevisionRelation::Different, completed, &applied, &migration, &error) &&
			completed["weight"] == 2.0 && completed["enabled"] == true && !completed.contains("removed") &&
			applied.size() == 1 && applied[0].path.ToString() == "/weight" &&
			migration.applied == 1 && migration.staleMissingPath == 1 && migration.staleIncompatible == 1,
		...
		Check(!ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, arrayElement,
			ActorImprintOverrideRevisionRelation::Different, completed, nullptr, nullptr, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::ArrayElementPath,
		...
		Check(!ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, wrongFloat,
			ActorImprintOverrideRevisionRelation::Same, completed, nullptr, nullptr, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::IncompatibleRepresentation,
		...
		Check(ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, nullPrimitive,
			ActorImprintOverrideRevisionRelation::Different, completed, nullptr, &migration, &error) &&
			migration.staleIncompatible == 1 && completed["weight"] == defaults["weight"],
		...
		Check(ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, nullReferences,
			ActorImprintOverrideRevisionRelation::Same, completed, &applied, &migration, &error) &&
			applied.size() == 2 && completed["target"].is_null() && completed["texture"].is_null(),
		...
		Check(!ActorImprintPropertyOverrides::Merge(ProbeMetadata(), defaults, conflicting,
			ActorImprintOverrideRevisionRelation::Different, completed, nullptr, nullptr, &error) &&
			error.code == ActorImprintPropertyOverrideErrorCode::InvalidPath,
