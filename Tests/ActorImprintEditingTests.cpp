...
namespace
{
	...
	struct Fixture
	{
		...
		Fixture()
		{
			...
			destination.close();
			Check(MetaFile::Save((root / "Editable.imprint").string(), assetGuid), "Editing fixture writes stable asset metadata");
			Check(assets.Initialize(root.string(), nullptr, nullptr), "Editing fixture catalog initializes");
		...
		std::unique_ptr<ActorImprintEditingContext> Open()
		{
			...
			auto context = ActorImprintEditingContext::Open(
				assetGuid, assets, system, engineContext);
			if (!context) std::cerr << "Operation failed\n";
			return context;
		}
	};
	...
	void OpenAndPolicy()
	{
		...
		Check(roots.size() == 1 && roots.front()->GetName() == "Root", "Working Scene expands the complete definition with one root");
		Check(scene->GetImprintInstances().GetInstances().empty(), "Working Scene expansion does not create runtime Instance provenance");
		...
		bool result = false;
		Check(scene->CanAddRootActor() == false &&
			!scene->AddRootActor(ActorFactory::CreateEmptyActor({})),
			"Single-root policy matches preflight and execution for a second root");
		Actor* root = roots.front();
		Check(scene->CanDestroy(root) == false &&
			!scene->RemoveActor(root, true),
	...
	void CommandIdentityAndSnapshot()
	{
		...
		Check(document->ExecuteCommand(std::move(create)), "Existing CreateActorCommand executes through the editing wrapper");
		...
		Check(scene->CanReparent(child, nullptr) == false, "Working Scene refuses moving a child outside the closed subtree");

		Check(document->Undo() &&
			editing->GetObjectMap().FindActor(childGuid) == 0 &&
			editing->GetObjectMap().GetNextLocalObjectId() == 14,
		...
		Check(child &&
			editing->GetObjectMap().FindActor(childGuid) == childId &&
		...
		Check(!child->GetComponentByClass<Camera>() &&
			editing->GetObjectMap().FindComponent(cameraId) == nullptr &&
			editing->GetObjectMap().GetNextLocalObjectId() == 15,
		...
		Check(scene->CanReferenceActor(foreign->GetGuid()) == false &&
		...
		auto snapshot = editing->CaptureSnapshot(saved);
		Check(snapshot != nullptr &&
			saved["rootActorLocalObjectId"] == 10 &&
			saved["nextLocalObjectId"] == 15 &&
			saved["actors"].size() == 2,
			"Working Scene and ObjectMap produce a complete immutable snapshot");

		auto roundTrip = ActorImprintAssetDeserializer::Deserialize(saved, nullptr);
		Check(roundTrip != nullptr, "Captured editing snapshot passes the ET-08 asset validator");

		Check(document->ExecuteCommand(std::make_unique<RemoveComponentCommand>(
			scene, childGuid, "Camera", 0)),
			"Existing RemoveComponentCommand executes through the wrapper");
		Check(!editing->GetObjectMap().FindComponent(cameraId), "Component removal clears its runtime binding");
		Check(document->Undo(), "Component removal Undo restores the Component");
		camera = child->GetComponentByClass<Camera>();
		Check(camera && editing->GetObjectMap().FindComponent(camera) == cameraId, "Component removal Undo preserves its LocalObjectID");
		...
		Check(editing->GetObjectMap().FindActor(childGuid) == 0, "Subtree deletion removes Actor and Component bindings together");
	...
	void DocumentIsolationAndDeduplication()
	{
		...
		Check(firstId.IsValid() &&
			duplicateId == firstId &&
			manager.GetDocumentCount() == 1 &&
	...
	void DefaultRootModel()
	{
		...
		auto definition = ActorImprintEditingSnapshot::CreateDefault(
			fixture.assets, fixture.engineContext, serialized);
		...
		Check(root.parentId == InvalidLocalObjectId &&
			root.properties["name"] == "Root" &&
			root.components.size() == 1 &&
			root.components.front().id == 2 &&
			root.components.front().typeName == "Transform",
			"Default model is a root Actor with only its required Transform");

		Check(ActorImprintAssetDeserializer::Deserialize(serialized, nullptr) != nullptr,
	...
	void SaveAndReload()
	{
		...
		Check(document->ExecuteCommand(std::make_unique<ChangeActorTagCommand>(
			scene, root->GetGuid(), savedTag)) &&
			root->GetTag() == savedTag,
			"ActorImprint document applies the shared Actor tag command");
		Check(document->Undo() &&
			root->GetTag() == TAG_NONE &&
			document->Redo() &&
			root->GetTag() == savedTag,
			"ActorImprint document preserves tag changes through Undo and Redo");
		Check(document->Save() && !document->IsDirty(), "Validated atomic save marks the document clean only after success");
		Check(!HasTransactionResidue(fixture.root), "Successful replacement removes its same-directory transaction files");

		auto diskDefinition = ActorImprintAssetDeserializer::Load(
			(fixture.root / "Editable.imprint").string(), nullptr);
		Check(diskDefinition &&
			diskDefinition->GetActors().front().properties["name"] == "SavedRoot" &&
		...
		if (changes.size() == 1)
		{
			...
			Check(reload.status == ActorImprintReloadStatus::Reloaded &&
				reload.previousRevision == originalRevision &&
				reload.currentRevision == savedRevision &&
		...
		auto equivalent = ActorImprintAssetDeserializer::Load(
			(fixture.root / "Editable.imprint").string(), nullptr);
		Check(equivalent &&
			equivalent->GetRevision() == savedRevision &&
	...
	void AtomicSaveFailurePreservesState()
	{
		...
		const bool saved = document->GetEditingContext()->Save();
		if (lock != INVALID_HANDLE_VALUE) CloseHandle(lock);
		Check(!saved, "Destination replacement failure is reported as an atomic save failure");
		...
		Check(!HasTransactionResidue(fixture.root), "Failed replacement recovers and removes temporary transaction files");
	}
	...
	void PreparedSaveRollbackPreservesAuthoringState()
	{
		...
		Actor* root = document->GetWorkingScene()->GetRootActors().front();
		Check(document->ExecuteCommand(std::make_unique<RenameActorCommand>(
			document->GetWorkingScene(), root->GetGuid(), "RollbackRoot")) &&
			document->IsDirty(),
			"Prepared save rollback fixture creates a dirty authoring change");
		Check(document->PrepareSave() &&
			document->GetEditingContext()->HasPendingSave() &&
			document->IsDirty() &&
			ReadText(path) != originalBytes,
		...
		Check(document->Save() && !document->IsDirty(), "A rolled-back prepared save can be retried through the normal Document Save path");
	}
	...
	void ProjectTagManagement()
	{
		...
		Check(static_cast<bool>(TagManagementWorkflow::Delete("WorkflowRenamed", settings, settingsPath,
			fixture.assets, documents)) &&
			settings.GetUserTags().empty(),
			"Project Tag workflow deletes an unused Tag");

		Check(static_cast<bool>(TagManagementWorkflow::Create("WorkflowUsed", settings, settingsPath)), "Usage fixture creates a project Tag");
		auto context = fixture.Open();
		...
		Check(static_cast<bool>(TagManagementWorkflow::Create("DiskUsed", settings, settingsPath)), "Saved usage fixture creates a project Tag");
