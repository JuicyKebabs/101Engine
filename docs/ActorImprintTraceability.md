...
| U-06 | Instance structure is a runtime Scene invariant; whole-root destruction and deferred Registry cleanup form one lifecycle | ET-11, ET-17 | `SceneStructuralMutation.cpp`, `SceneBase` bool mutation checks, Registry destroying state | `ActorImprintStructuralMutationTests`, command regressions, `ActorImprintWorkflowTests` |
