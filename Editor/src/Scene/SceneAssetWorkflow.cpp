...
namespace
{
	...
	{
		if (error) *error = {path.generic_string(), std::move(message)};
		return false;
	}

	bool ExistsCaseInsensitive(const fs::path& directory, const fs::path& filename,
		const fs::path& ignored = {})
	{
		...
		for (const auto& item : fs::directory_iterator(directory, error))
		{
			...
			if (!ignored.empty() && fs::equivalent(item.path(), ignored, error)) { error.clear(); continue; }
...
bool SceneAssetWorkflow::NormalizeName(std::string_view input, std::string& outName,
	SceneAssetWorkflowError* outError)
{
	outName.assign(input);
	while (!outName.empty() && std::isspace(static_cast<unsigned char>(outName.front()))) outName.erase(outName.begin());
	while (!outName.empty() && std::isspace(static_cast<unsigned char>(outName.back()))) outName.pop_back();
	if (outName.size() > 6)
	{
		std::string suffix = outName.substr(outName.size() - 6);
		std::transform(suffix.begin(), suffix.end(), suffix.begin(),
			[](unsigned char c) { return static_cast<char>(std::tolower(c)); });
		if (suffix == ".scene") outName.resize(outName.size() - 6);
	}
	...
	static constexpr const char* reserved[] = {"CON", "PRN", "AUX", "NUL", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9"};
	for (const char* value : reserved) if (upper == value) return Fail(outError, {}, "Scene name is reserved by Windows.");
	return true;
}
...
bool SceneAssetWorkflow::Create(std::string_view name, SceneBase& scene, AssetManager& assets,
	Guid& outGuid, std::string& outPath, SceneAssetWorkflowError* outError)
{
	if (outError) *outError = {};
	outGuid = {};
	outPath.clear();
	std::string normalizedName;
	if (!NormalizeName(name, normalizedName, outError)) return false;
	const fs::path directory = fs::path(assets.GetAssetRoot()) / "scenes";
	std::error_code error;
	fs::create_directories(directory, error);
	if (error) return Fail(outError, directory, error.message());
...
bool SceneAssetWorkflow::Rename(const Guid& guid, std::string_view newName, AssetManager& assets,
	std::string& outPath, SceneAssetWorkflowError* outError)
{
	if (outError) *outError = {};
	outPath.clear();
	...
	std::string normalizedName;
	if (!NormalizeName(newName, normalizedName, outError)) return false;
	...
	if (source.filename() == destination.filename()) { outPath = source.string(); return true; }
	if (ExistsCaseInsensitive(source.parent_path(), destination.filename(), source))
		return Fail(outError, destination, "A Scene with this name already exists.");
	if (!fs::exists(sourceMeta)) return Fail(outError, sourceMeta, "Scene metadata is missing.");

	const fs::path staging = source.parent_path() / (source.filename().wstring() + L"." +
		fs::path(GuidGenerator::Generate().ToString()).wstring() + L".rename");
	const fs::path stagingMeta = staging.string() + ".meta";
	if (!Move(source, staging)) return Fail(outError, source, "Could not stage the Scene for rename.");
	if (!Move(sourceMeta, stagingMeta)) { Move(staging, source); return Fail(outError, sourceMeta, "Could not stage Scene metadata for rename."); }
	if (!Move(staging, destination) || !Move(stagingMeta, destinationMeta))
	{
		if (fs::exists(destination)) Move(destination, staging);
		if (fs::exists(destinationMeta)) Move(destinationMeta, stagingMeta);
		Move(staging, source);
