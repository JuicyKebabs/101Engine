	...
	bool activate)
{
	...
	if (sourceAsset.IsValid())
	{
		for (const Entry& entry : m_documents)
		{
			if (entry.document->GetType() != document->GetType() ||
				entry.document->GetSourceAssetGuid() != sourceAsset) continue;
			if (activate) m_activeDocumentId = entry.id;
			return entry.id;
		}
	}
	...
	if (activate || !m_activeDocumentId.IsValid()) m_activeDocumentId = id;
	return id;
}

bool EditorDocumentManager::ActivateDocument(EditorDocumentId id)
{
	if (FindEntry(id) == m_documents.end()) return false;
...
bool EditorDocumentManager::ActivateDocument(IEditorDocument* document)
{
	...
	for (const Entry& entry : m_documents)
	{
		if (entry.document.get() != document) continue;
...
IEditorDocument* EditorDocumentManager::FindFirst(EditorDocumentType type)
{
	for (Entry& entry : m_documents)
		if (entry.document->GetType() == type) return entry.document.get();
	return nullptr;
}
...
{
	if (!assetGuid.IsValid()) return false;
...
{
	auto entry = FindEntry(id);
	if (entry == m_documents.end()) return EditorDocumentCloseResult::NotFound;
	...
	if (wasActive)
	{
		if (m_documents.empty()) m_activeDocumentId = {};
		else m_activeDocumentId = m_documents[std::min(index, m_documents.size() - 1)].id;
	}
	...
	const AssetChange& change)
{
	...
	ActorImprintReloadResult result = system.Reload(change,
		std::span<std::unique_ptr<SceneBase>* const>(owners));
...
std::vector<std::unique_ptr<SceneBase>*> EditorDocumentManager::CollectWorkingSceneOwnerSlots()
{
	...
	{
		auto* owner = entry.document->GetWorkingSceneOwnerSlot();
		if (owner && *owner) owners.push_back(owner);
	}
	return owners;
}
	...
	std::span<const std::size_t> replacedIndices)
{
	...
	{
		if (index >= owners.size()) continue;
		for (Entry& entry : m_documents)
		{
			if (entry.document->GetWorkingSceneOwnerSlot() != owners[index]) continue;
			entry.document->ClearCommandHistory();
