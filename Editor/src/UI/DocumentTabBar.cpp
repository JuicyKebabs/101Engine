...
void DocumentTabBar::Render(std::span<const EditorDocumentInfo> documents,
	const Callbacks& callbacks)
{
	...
	for (const EditorDocumentInfo& document : documents)
	{
		...
		if (document.isDirty) label += " *";
		label += "###EditorDocument" + std::to_string(document.id.value);
		const ImGuiTabItemFlags flags = isSynchronizingSelection && document.isActive
		...
		if (!open) DispatchClose(callbacks, document.id);
	}
	...
	{
		if (documents.empty()) m_visibleDocumentId = {};
		return;
	}
