...
void SceneAssetPanel::Render(const AssetManager& assets, const Guid& startupSceneGuid,
	const Callbacks& callbacks)
{
	if (ImGui::Begin("Scenes"))
	{
		...
		for (const AssetEntry& entry : entries)
		{
			...
			const bool isStartup = entry.guid == startupSceneGuid;
			...
			if (ImGui::BeginPopupContextItem("SceneItemContext"))
			{
				...
				if (ImGui::MenuItem("Set as Startup Scene", nullptr, isStartup,
					callbacks.canModify && !isStartup) && callbacks.onSetStartup)
					callbacks.onSetStartup(entry.guid);
				ImGui::EndPopup();
			}
			ImGui::PopID();
		}
		if (entries.empty()) ImGui::TextDisabled("No Scene assets found.");
	...
	if (m_openRenamePopup) { ImGui::OpenPopup("Rename Scene"); m_openRenamePopup = false; }
	if (ImGui::BeginPopupModal("Rename Scene", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
	{
		...
		ImGui::SameLine();
		if (ImGui::Button("Cancel", ImVec2(120, 0))) { m_renameGuid = {}; ImGui::CloseCurrentPopup(); }
		ImGui::EndPopup();
	}
}
