...
ChangeActorTagCommand::ChangeActorTagCommand(
	SceneBase* scene, const Guid& actorGuid, TagId newTag)
...
bool ChangeActorTagCommand::Execute()
{
	...
	if (!m_hasExecuted)
	{
		...
		if (m_oldTag == m_newTag) return false;
		actor->SetTag(m_newTag);
		m_hasExecuted = true;
		return true;
	}

	if (actor->GetTag() != m_oldTag) return false;
	actor->SetTag(m_newTag);
	return true;
}
...
{
	if (!m_scene || !m_hasExecuted) return false;
	Actor* actor = m_scene->ResolveActor(m_actorGuid);
	if (!actor || actor->IsDestroyed() || actor->GetOwner() != m_scene ||
		actor->GetTag() != m_newTag) return false;
