...
ActorImprintEditingCommand::ActorImprintEditingCommand(
	SceneBase& scene,
...
{
	if (!m_scene || !m_objectMap || !m_command)
	{
		return false;
	}
	...
	if (!m_hasExecuted)
	{
		if (!m_objectMap->CaptureSnapshot(*m_scene, m_before))
		{
			return false;
		}
...
{
	if (!m_scene || !m_objectMap || !m_command || !m_hasExecuted)
	{
		return false;
	}
