...
{
	// Prepare transformation and color
	const Transform3D& colliderTransform = { collider.GetCurrentCenter(),collider.GetCurrentScale(),collider.GetRotation() };
	XMFLOAT4 objColor{};
...
//レイキャストの衝突状態を更新
void CollisionManager::UpdateRaycastCollisionState(CollisionData::RaycastSegment& ray)
{
	...
	for (auto& p : ray.currHitInfos)
	{
		...
		p.state = (prevSet.find(p.opponent) != prevSet.end())
			? CollisionData::COLLISION_STATE::COLLISION_STAY 
			: CollisionData::COLLISION_STATE::COLLISION_ENTER;
	}
	...
	for (auto& p : ray.prevHitInfos)
	{
		...
		{
			CollisionData::RaycastHitInfo exitInfo = p;
			exitInfo.state = CollisionData::COLLISION_STATE::COLLISION_EXIT;
