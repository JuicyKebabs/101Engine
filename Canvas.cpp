...
)
{
	...
	using args = float;
	uint64_t id = m_fadeStartEventID = EventManager::GetInstance()->Subscribe<args>(
		EventType::START_FADE_IN,
		[this](std::shared_ptr<args> data) {
			StartFadeIn(*data);
		}
	);
	m_eventDataList.push_back({ EventType::START_FADE_IN, id });

	uint64_t id2 = m_fadeEndEventID = EventManager::GetInstance()->Subscribe<args>(
		EventType::START_FADE_OUT,
		[this](std::shared_ptr<args> data) {
			StartFadeOut(*data);
		}
	);
	m_eventDataList.push_back({ EventType::START_FADE_OUT, id2 });
