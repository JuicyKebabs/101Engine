...
namespace
{
	...
	bool ReadGuid(const nlohmann::json& root, const char* section, Guid& outGuid,
		std::string* error)
	{
		if (!root.contains(section)) return true;
		const auto& object = root[section];
		if (!object.is_object()) return Fail(error, std::string(section) + " must be an object.");
		if (!object.contains("startupSceneGuid") || object["startupSceneGuid"].is_null()) return true;
	...
	{
		if (!root.contains(section) || !root[section].is_object()) root[section] = nlohmann::json::object();
		if (guid.IsValid()) root[section]["startupSceneGuid"] = guid.ToString();
		else root[section].erase("startupSceneGuid");
	}

	bool ReadTags(const nlohmann::json& root, std::vector<std::string>& outTags, std::string* error)
	{
		if (!root.contains("tags")) return true;
		if (!root["tags"].is_array()) return Fail(error, "tags must be an array of strings.");
		std::vector<std::string> input;
		for (const auto& value : root["tags"])
		{
			if (!value.is_string()) return Fail(error, "tags must contain only strings.");
...
bool ProjectSettings::Load(const std::filesystem::path& path, ProjectSettings& outSettings,
	std::string* outError)
{
	...
	try
	{
		...
		if (!stream) return Fail(outError, "Could not open project settings.");
		const nlohmann::json root = nlohmann::json::parse(stream);
		if (!root.is_object()) return Fail(outError, "Project settings root must be an object.");
		...
		if (!ReadGuid(root, "editor", candidate.m_editorStartupSceneGuid, outError) ||
			...
			!ReadTags(root, candidate.m_userTags, outError)) return false;
...
bool ProjectSettings::Save(const std::filesystem::path& path, std::string* outError) const
{
	...
	try
	{
		...
		if (!TagRegistry::ValidateUserTagSet(m_userTags, tags, outError)) return false;
		root["tags"] = tags;
			 ...
			 std::filesystem::path(GuidGenerator::Generate().ToString()).wstring() + L".tmp");
		{
			...
			if (!output) return Fail(outError, "Could not create temporary project settings.");
			output << root.dump(2);
			output.flush();
			if (!output) { std::filesystem::remove(temporary); return Fail(outError, "Could not write project settings."); }
		}
