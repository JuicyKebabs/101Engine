...
TagId TagRegistry::Register(std::string_view tagName)
{
	...
	if (it == m_tags.end() || it->second == tagName) m_tags[id] = std::string(tagName);
	return id;
}

bool TagRegistry::NormalizeUserTagName(std::string_view input, std::string& output, std::string* outError)
{
	auto fail = [&](const char* message) { if (outError) *outError = message; return false; };
	...
	if (output.empty()) return fail("Tag name must not be empty.");
	if (output.size() > 64) return fail("Tag name must not exceed 64 UTF-8 bytes.");
	for (unsigned char c : output) if (c < 0x20 || c == 0x7f) return fail("Tag name must not contain control characters.");
	if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, output.data(), static_cast<int>(output.size()), nullptr, 0) == 0)
...
bool TagRegistry::NamesEqualCaseInsensitive(std::string_view left, std::string_view right)
{
	...
	if ((left.empty() || !wideLeft.empty()) && (right.empty() || !wideRight.empty()))
	{
		return CompareStringOrdinal(wideLeft.data(), static_cast<int>(wideLeft.size()),
			wideRight.data(), static_cast<int>(wideRight.size()), TRUE) == CSTR_EQUAL;
	}
	return left == right;
}
...
bool TagRegistry::RegisterUserTag(std::string_view tagName, TagId* outId, std::string* outError)
{
	...
	if (!NormalizeUserTagName(tagName, normalized, outError)) return false;
	for (const auto& [id, name] : m_tags)
	{
		if (NamesEqualCaseInsensitive(name, normalized))
		{
			if (name != normalized) { if (outError) *outError = "Tag name differs only by letter case."; return false; }
			if (outId) *outId = id;
			return true;
		}
	}
	...
	{
		if (outError) *outError = "Tag name has an FNV-1a collision with '" + collision->second + "'.";
		return false;
	}
...
bool TagRegistry::ValidateUserTagSet(const std::vector<std::string>& input,
	std::vector<std::string>& normalized, std::string* outError)
{
	...
	{
		std::string name;
		if (!NormalizeUserTagName(value, name, outError)) return false;
		if (validation.ContainsName(name))
		{
			if (outError) *outError = "Tag list contains a duplicate name.";
			return false;
		}
		if (!validation.RegisterUserTag(name, nullptr, outError)) return false;
...
bool TagRegistry::UnregisterUserTag(std::string_view tagName)
{
	...
	const auto it = m_tags.find(id);
	if (it == m_tags.end() || it->second != tagName) return false;
	m_tags.erase(it);
	return true;
}
...
TagId TagRegistry::GetId(std::string_view tagName)
{
	const TagId id = CalcTagId(tagName);
	const auto it = m_tags.find(id);
	if (it == m_tags.end()) m_tags[id] = std::string(tagName);
	return id;
}
...
std::vector<std::pair<TagId, std::string>> TagRegistry::GetRegisteredTags() const
{
	...
	for (const auto& [id, name] : m_tags)
	{
		if (id != TAG_NONE) tags.emplace_back(id, name);
	}
