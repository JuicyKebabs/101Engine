...
namespace
{
		...
		UnknownPropertyPolicy unknownPropertyPolicy)
	{
		if (!source.is_object())
		{
			DBG("Expected a reflected property object.");
			return false;
		}
		...
		for (auto member = source.begin(); member != source.end(); ++member)
		{
			...
			DBG("Unknown reflected property: %s", member.key().c_str());
			return false;
		}
		...
		for (const auto& [name, childSchema] : schema.children)
		{
			...
			if (child == source.end())
			{
				...
				DBG("Required reflected property is missing.");
				return false;
			}

			if (childSchema.property)
			{
				if (!childSchema.children.empty())
				{
					return false;
				}
			}
			else if (!ValidateSchema(*child, childSchema, unknownPropertyPolicy))
			{
				return false;
			}
		}
	...
	bool SetValueAtPath(json& root, const PropertyPath& path, json value)
	{
		...
		if (object->contains(members.back()))
		{
			return false;
		}
	...
	bool DeserializeFloatArray(const json& source, std::array<float, Size>& outValues)
	{
		...
		{
			if (!TryReadFloat(source[i], outValues[i]))
			{
				return false;
			}
		}
		...
		ReflectionRestoreContext context)
	{
		...
		case PropertyLogicalType::Vector2:
		{
			...
			if (!DeserializeFloatArray(source, values))
			{
				return false;
			}
		...
		case PropertyLogicalType::ActorReference:
		{
			if (!context.actorReferenceCodec)
			{
				return false;
			}
	...
	ReflectionSaveContext context)
{
	...
	for (const PropertyMetadata* property : GetSerializableProperties(metadata))
	{
		...
		{
			DBG("Failed to read a serializable property.");
			return false;
		}

		if (!SerializePropertyValue(*property, value, serializedValue, context))
		{
			DBG("Failed to encode a serializable property.");
			return false;
		}
		...
		{
			DBG("Property path conflicts with another serialized property.");
			return false;
		}
	}
	...
	ReflectionRestoreContext context)
{
	...
	if (!json.is_object())
	{
		DBG("Reflection data must be a JSON object.");
		return false;
	}
	...
	const bool schemaValid =
		schemaBuilt &&
		ValidateSchema(json, schema, context.unknownPropertyPolicy);

	if (!schemaValid)
	{
		if (!schemaBuilt)
		{
			DBG("Registered reflection property paths conflict.");
		}
	...
	for (const PropertyMetadata* property : properties)
	{
		...
		if (!entry)
		{
			...
			if (serialization && serialization->requirement == PropertyRequirement::Optional)
			{
				continue;
			}

			DBG("A required property is missing.");
			return false;
		}
		...
		{
			DBG("Serialized property has an invalid representation.");
			return false;
		}

		if (!property->ValidateValue(value))
		{
			DBG("Serialized property is outside its supported value domain.");
			return false;
		}
	...
	for (const auto& propertyAndValue : values)
	{
		...
		{
			DBG("Failed to capture a property before deserialization.");
			return false;
		}
	...
	auto Rollback = [&]()
	{
		...
		while (appliedCount > 0)
		{
			...
			if (!previous.first->Write(objectType, object, previous.second))
			{
				restored = false;
			}
		}
	...
	for (const auto& propertyAndValue : values)
	{
		...
		if (!property->Write(objectType, object, propertyAndValue.second))
		{
			...
			if (!restored)
			{
				DBG("Deserialized property rollback failed.");
			}

			DBG("Failed to apply a deserialized property.");
			return false;
		}
	}

	if (!metadata.Validate(objectType, object))
	{
		const bool restored = Rollback();

		if (!restored)
		{
			DBG("Type validation failed and rollback was incomplete.");
			return false;
		}
