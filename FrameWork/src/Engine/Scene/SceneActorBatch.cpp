...
void SceneActorBatch::Stage(std::vector<std::unique_ptr<Actor>> actors)
{
	...
	std::size_t size = 0;
	for (auto h : m_handles) size = (std::max)(size, static_cast<std::size_t>(h.index) + 1);
...
bool SceneActorBatch::ResolveAndValidate(Actor* root, Actor* externalParent, std::string& error)
{
	...
	for (const auto handle : m_handles)
	{
		...
		for (Component* component : actor->GetAllComponents())
		{
			...
			ReflectionError detail;
			if (!SerializeReflectedComponent(*component, properties, &m_candidate, &detail))
			{
			...
			{
				error = "Component reference resolution failed on Actor '" + actor->GetName() + "'.";
				return false;
			}
		}
	}
	...
	{
		error = "UI hierarchy requires a different Transform-family component.";
		return false;
	}
	return true;
}
...
void SceneActorBatch::PrepareCommit(Actor* root, Actor* externalParent)
{
	...
	if (externalParent) externalParent->m_childHandles.reserve(externalParent->m_childHandles.size() + 1);
	m_root = root;
