...
{
	if (name.empty() || !scene || GetSceneElement(name))
	{
		return;
	}
...
{
	if (name.empty() || !sceneAssetGuid.IsValid() || GetSceneElement(name) || GetSceneElement(sceneAssetGuid))
	{
		return false;
	}

	if (m_context.pAssetManager && !ValidateSceneAsset(sceneAssetGuid))
	{
		return false;
	}
...
void SceneManager::Initialize(EngineContext& context)
{
	...
	if (m_currentSceneAssetGuid.IsValid())
	{
		ChangeScene(m_currentSceneAssetGuid);
	}
	else if (!m_currentSceneName.empty())
	{
		ChangeScene(m_currentSceneName);
	}
}
...
void SceneManager::Update(float deltaTime)
{
	...
	if (m_reservedSceneAssetGuid.IsValid())
	{
		ChangeScene(m_reservedSceneAssetGuid);
	}
	else
	{
...
void SceneManager::ChangeScene(SceneElement& element)
{
	...
	{
		if (!ValidateSceneAsset(element.sceneAssetGuid))
		{
			ClearReservation();
			return;
		}
		...
		if (!load)
		{
			DBG("SceneManager: Failed to load scene from '%s' at '%s': %s", path.c_str(), load.error.path.c_str(),
	...
	if (previousCurrent && previousCurrent != nextScene)
	{
		previousCurrent->Finalize();
	}

	if (replacedTarget && replacedTarget.get() != previousCurrent)
	{
		replacedTarget->Finalize();
	}
...
{
	if (!sceneAssetGuid.IsValid() || !m_context.pAssetManager)
	{
		return false;
	}
...
SceneElement* SceneManager::GetSceneElement(const Guid& sceneAssetGuid)
{
	for (auto& element : m_sceneElements)
	{
		if (element.source == SceneElement::Source::Asset && element.sceneAssetGuid == sceneAssetGuid)
		{
			return &element;
		}
	}
...
void SceneManager::SetViewportSize(UINT width, UINT height)
{
	...
	if (m_pCurrentScene)
	{
		m_pCurrentScene->SetViewportSize(width, height);
	}
}
