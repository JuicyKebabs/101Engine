...
void AudioManager::Terminate()
{
	...
	for (AudioHandle handle = 0; handle < AUDIO_MAX; ++handle)
	{
		UnloadImmediate(handle);
	}
...
{
	if (!m_pXaudio || fullPath.empty() || voiceCount == 0)
	{
	...
	if (valid)
	{
		...
		else if (formatChunk.cksize >= sizeof(PCMWAVEFORMAT))
		{
			...
			if (valid)
			{
				std::memcpy(&format, &pcm, sizeof(pcm));
				format.cbSize = 0;
			}
		}
		else
	...
	if (valid)
	{
		valid = mmioAscend(file, &formatChunk, 0) == MMSYSERR_NOERROR &&
	...
	for (uint32_t voiceIndex = 0; voiceIndex < voiceCount; ++voiceIndex)
	{
		...
		{
			for (IXAudio2SourceVoice* created : resource.sourceVoices)
			{
				created->DestroyVoice();
			}
...
void AudioManager::ProcessAudioCommands()
{
	for (const AudioCommand& command : m_commandQueue)
	{
		...
		case CommandType::Play:
			PlayImmediate(command.handle, command.loop);
			break;
		case CommandType::Stop:
			StopImmediate(command.handle);
			break;
		case CommandType::Unload:
			UnloadImmediate(command.handle);
			break;
		}
	}
