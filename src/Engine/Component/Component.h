...
public:
	struct InitDesc
	{
		std::string name = "NoName";	// Component name (optional, can be used for debugging or identification)
		InitDesc(const std::string& name = "Component") : name(name) {}
	};
...
public:
	...
	bool IsInitialized() const { return m_isInitialized; }		// Check if the component has been initialized
...
private:
	...
	bool m_isInitialized = false;	// Flag to check if the component has been initialized (Init function has been called at least once)
